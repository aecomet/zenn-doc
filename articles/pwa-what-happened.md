...
ただし、まだ整備しきれていない部分も多い。ツールチェーンの複雑性、Component Modelの学習コスト、デバッグ体験の未成熟など、プロダクション採用のハードルは下がり切っていません。「URLで配布できる、インストール不要の高性能アプリ」——PWAが夢見た世界を、WasmがJavaScriptと組み合わさる形で別のレイヤーから実現できるかどうか、その答えはまだ出ていません。
