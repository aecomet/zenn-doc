---
title: "MySQLの自己参照 ON DELETE CASCADE には“15階層の壁”がある（しかも設定変更不可）"
emoji: "🌲"
type: "tech"
...
ツリー構造（自己参照テーブル）を「親を消したら子孫も全部消える」ようにしたくて `ON DELETE CASCADE` を張ろうとしたら、想像以上に引っかかる点が多かったので、**公式ドキュメントと実測**でまとめます。

この記事では以下を扱います。

- `ON DELETE CASCADE` を自己参照テーブルに張る際の **4 つの落とし穴**（STORED 生成カラム制約・15 段上限・全消し失敗・トリガ非発火）
- 上限の **回避策と代替削除方法**の性能比較
- 深い階層に備えるための **ツリー表現3方式**（隣接リスト／materialized path／closure table）の使い分け
...
`parent_id` で親を指すだけのシンプルな自己参照テーブルです。ここでは「root かどうか」をインデックス用のフラグとして、`parent_id` から**生成カラム**で持たせています（実務でもよくある形）。
...
やりたいのは「**親を消したら子孫も全部消える**」こと。そのためにこの自己参照 FK を `ON DELETE CASCADE` にしたい——という前提で進めます。

## 自己参照ツリーはどこで使う？大規模ストレージとの違い

`parent_id` の自己参照（隣接リスト）は、アプリの DB 内に小〜中規模のツリーを持つときの定番です。
...
一方、**Google Drive や SharePoint のような大規模ストレージは、RDBMS の自己参照＋CASCADE では作られていない**と見られます（内部実装は非公開なので推測ですが、ストレージ自体が MySQL ではないはずです）。一般に、**メタデータと実体を分離**し、削除は**ソフトデリート＋非同期パージ**で行う構成です。

```mermaid
flowchart TB
    A["アプリ / API"]
    A --> M[("分散メタデータストア<br/>各アイテムが親参照を保持")]
    A --> O[("オブジェクトストレージ<br/>ファイル実体")]
    M -->|"削除はソフトデリート"| W["非同期ワーカー"]
    W -->|"子孫を順次パージ"| M
    W -->|"実体を削除"| O
```

両者のメリット・デメリットはこうなります。

| | RDBMS 自己参照（隣接リスト） | 専用の分散メタストア＋オブジェクトストレージ（大規模ドライブ型） |
|---|---|---|
| 向く規模 | 小〜中規模のアプリ内ツリー | 超大規模 |
| 実装コスト | 低（テーブル1つ） | 高（自前構築は非現実的） |
| トランザクション | 強い（単一 DB） | 緩い（結果整合になりがち） |
| 深い階層の一括削除 | 苦手（CASCADE は 15 段／再帰CTE が必要） | 得意（ソフトデリート＋非同期パージ） |
| メタデータと実体 | 同居しがち | 分離が前提 |

:::message
**規模の目安**: 分散メタストアが要るのは数億行〜やマルチリージョンが必要なときで、**数千万行程度までなら RDBMS 単体で十分**です（適切なインデックスがあれば InnoDB は数千万〜億行を捌けます）。分散ストアは“超大規模向けの別解”であり、多くのアプリでは過剰です。本記事の主題（自己参照＋CASCADE の課題）は、この RDBMS で扱う規模の話です。
:::

この記事で扱う「自己参照＋CASCADE」の課題が効いてくるのは前者です。大規模ストレージはそもそもこの土俵に乗らない設計になっている、と捉えると分かりやすいです。
...
**なぜ**: MySQL は「**STORED 生成カラムの計算元（base 列）**には、CASCADE / SET NULL などの参照アクション付き FK を張れない」という制約を持っています。`parent_id` が `is_root`（STORED）の base なので弾かれます。
...
**この「15」は `my.cnf` で変えられる？ → いいえ。** 設定値ではなく**仕様として 15 で固定**です。
